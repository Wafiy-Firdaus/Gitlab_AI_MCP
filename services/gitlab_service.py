...
class GitLabService:
    ...
    def resolve_url_or_ids(self, url: str | None, project_id: int | str | None, resource_id: int | None) -> tuple[int | str | None, int | str | None]:
        """
        ...
        if url:
            ...
            if parsed:
                return parsed["project_path"], int(parsed["resource_id"]) if parsed["resource_id"].isdigit() else parsed["resource_id"]
        
        return project_id, resource_id

    async def triage_job_log_locally(self, project_id: int | str, job_id: int) -> dict[str, Any]:
        """
    ...
    async def analyze_failed_job(self, project_id: int | str, job_id: int) -> dict[str, Any]:
        """
    ...
    async def list_projects(self, search: str | None = None, group_id: int | str | None = None, fields: list[str] | None = None, limit: int = 20) -> dict[str, Any]:
        ...
        key_findings = [f"{p.get('name', 'N/A')} (ID: {p.get('id', 'N/A')}, Path: {p.get('path_with_namespace', p.get('path', 'N/A'))})" for p in projects[:5]]
        if len(projects) > 5:
    ...
    async def list_group_projects(self, group_id: int | str, fields: list[str] | None = None, limit: int = 20) -> dict[str, Any]:
        ...
        key_findings = [f"{p.get('name', 'N/A')} (ID: {p.get('id', 'N/A')}, Path: {p.get('path_with_namespace', p.get('path', 'N/A'))})" for p in projects[:10]]
    ...
    async def list_all_issues(self, state: str = "opened", scope: str = "assigned_to_me") -> dict[str, Any]:
        issues = await self.client.list_all_issues(state=state, scope=scope)
        
        summary = f"Found {len(issues)} {state} issues (Scope: {scope})"
        key_findings = [f"#{i['iid']} ({i['references']['full']}): {i['title']}" for i in issues[:10]]
        if len(issues) > 10:
    ...
    async def list_project_issues(self, project_id: int | str, state: str = "opened") -> dict[str, Any]:
    ...
    async def list_project_merge_requests(self, project_id: int | str, state: str = "opened") -> dict[str, Any]:
        ...
        key_findings = [f"!{mr['iid']}: {mr['title']} ({mr['source_branch']} -> {mr['target_branch']})" for mr in mrs[:5]]
        if len(mrs) > 5:
    ...
    async def get_issue_details(self, project_id: int | str, issue_iid: int) -> dict[str, Any]:
    ...
    async def create_issue(
        ...
        if assignee_ids: data["assignee_ids"] = assignee_ids
    ...
    async def get_merge_request_details(self, project_id: int | str, mr_iid: int, include_jobs: bool = False) -> dict[str, Any]:
    ...
    async def update_merge_request(self, project_id: int | str, mr_iid: int, **kwargs) -> dict[str, Any]:
        data = {k: v for k, v in kwargs.items() if v is not None}
    ...
    async def list_issue_notes(self, project_id: int | str, issue_iid: int) -> dict[str, Any]:
    ...
    async def list_merge_request_notes(self, project_id: int | str, mr_iid: int) -> dict[str, Any]:
    ...
    async def get_merge_request_diffs(self, project_id: int | str, mr_iid: int, paths: list[str] | None = None) -> dict[str, Any]:
        ...
        key_findings = [f"{d['new_path']} ({'+' if d['new_file'] else 'modified'})" for d in diffs[:10]]
    ...
    async def list_repository_files(self, project_id: int | str, path: str = "", ref: str = "main") -> dict[str, Any]:
    ...
    async def get_file_content(self, project_id: int | str, file_path: str, ref: str = "main", include_raw: bool = False) -> dict[str, Any]:
    ...
    async def get_raw_file_content(self, project_id: int | str | None = None, file_path: str | None = None, ref: str = "main", raw_url: str | None = None) -> dict[str, Any]:
        if raw_url:
    ...
    async def get_multiple_files(self, project_id: int | str, file_paths: list[str], ref: str = "main", include_metadata: bool = False) -> dict[str, Any]:
        """Fetch multiple files in parallel for efficiency."""
        tasks = [self.get_file_content(project_id, path, ref=ref, include_raw=include_metadata) for path in file_paths]
        ...
        key_findings = [f"Successfully fetched: {', '.join([r['path'] for r in processed_results])}"]
    ...
    async def get_branches(self, project_id: int | str, branches: list[str]) -> dict[str, Any]:
        ...
        key_findings = [f"Branch '{r['name']}': HEAD={r['head_sha'][:8]}" for r in processed_results[:10]]
        if errors:
    ...
    async def commit_file(self, project_id: int | str, file_path: str, branch: str, content: str, commit_message: str, is_new: bool = True) -> dict[str, Any]:
        if is_new:
            result = await self.client.create_repository_file(project_id, file_path, branch, content, commit_message)
        else:
            result = await self.client.update_repository_file(project_id, file_path, branch, content, commit_message)
            
        summary = f"Successfully {'created' if is_new else 'updated'} '{file_path}' on branch '{branch}'"
    ...
    async def global_search(self, scope: str, search: str, group_id: int | str | None = None) -> dict[str, Any]:
        ...
        if scope == "projects":
            key_findings = [f"Project: {r['name_with_namespace']} (ID: {r['id']})" for r in results[:10]]
        elif scope == "issues":
            key_findings = [f"Issue #{r['iid']}: {r['title']} (Project ID: {r['project_id']})" for r in results[:10]]
        elif scope == "merge_requests":
            key_findings = [f"MR !{r['iid']}: {r['title']} (Project ID: {r['project_id']})" for r in results[:10]]
        else:
    ...
    async def list_pipelines(self, project_id: int | str) -> dict[str, Any]:
        ...
        key_findings = [f"ID: {p['id']} - Status: {p['status']} (Ref: {p['ref']})" for p in pipelines[:5]]
    ...
    async def list_pipeline_jobs(self, project_id: int | str, pipeline_id: int) -> dict[str, Any]:
        ...
        key_findings = [f"Job: {j['name']} (ID: {j['id']}) - Status: {j['status']}" for j in jobs[:10]]
    ...
    async def get_pipeline_bridges(self, project_id: int | str, pipeline_id: int) -> dict[str, Any]:
        ...
        key_findings = [f"Bridge: {b['name']} (ID: {b['id']}) - Downstream: {b.get('downstream_pipeline', {}).get('id', 'N/A')}" for b in bridges[:10]]
    ...
    async def get_job_log(self, project_id: int | str, job_id: int) -> dict[str, Any]:
        log_content = await self.client.get_job_log(project_id, job_id)
        log_lines = log_content.splitlines()
        tail_log = "\n".join(log_lines[-100:])
        
        summary = f"Fetched trace log for job {job_id} (tail: 100 lines)"
        key_findings = [f"Total log lines: {len(log_lines)}", "Included tail of the log for debugging."]
    ...
    async def list_merge_request_discussions(self, project_id: int | str, mr_iid: int, unresolved_only: bool = False, include_system: bool = True) -> dict[str, Any]:
        ...
        key_findings = [f"Discussion {d['id'][:8]}: {len(d['notes'])} notes" for d in filtered_discussions[:5]]
    ...
    async def list_merge_request_discussion_summaries(self, project_id: int | str, mr_iid: int, unresolved_only: bool = False) -> dict[str, Any]:
        discussions = await self.client.get_merge_request_discussions(project_id, mr_iid)
    ...
    async def resolve_merge_request_discussion(self, project_id: int | str, mr_iid: int, discussion_id: str, resolved: bool = True) -> dict[str, Any]:
        result = await self.client.resolve_merge_request_discussion(project_id, mr_iid, discussion_id, resolved)
        
        summary = f"Successfully {'resolved' if resolved else 'unresolved'} discussion {discussion_id}"
        key_findings = [f"MR !{mr_iid}", f"Discussion ID: {discussion_id}", f"Status: {'Resolved' if resolved else 'Open'}"]
    ...
    async def retry_pipeline(self, project_id: int | str, pipeline_id: int) -> dict[str, Any]:
        ...
        key_findings = [f"Pipeline ID: {result['id']}", f"Status: {result['status']}", f"Ref: {result['ref']}"]
    ...
    async def get_job_artifact_file(self, project_id: int | str, job_id: int, artifact_path: str) -> dict[str, Any]:
        ...
        key_findings = [f"File: {artifact_path}", f"Job ID: {job_id}", f"Content Length: {len(content)} bytes"]
    ...
    async def list_repository_commits(self, project_id: int | str, ref_name: str | None = None) -> dict[str, Any]:
    ...
    async def get_file_blame(self, project_id: int | str, file_path: str, ref: str = "main") -> dict[str, Any]:
    ...
    async def list_project_labels(self, project_id: int | str) -> dict[str, Any]:
        ...
        key_findings = [f"{l['name']} (Color: {l['color']})" for l in labels[:10]]
    ...
    async def create_project_label(self, project_id: int | str, name: str, color: str, description: str | None = None) -> dict[str, Any]:
        ...
        key_findings = [f"Name: {label['name']}", f"Color: {label['color']}", f"Description: {label.get('description', 'N/A')}"]
    ...
    async def list_project_environments(self, project_id: int | str) -> dict[str, Any]:
        ...
        key_findings = [f"{e['name']} (Slug: {e['slug']}) - External URL: {e.get('external_url', 'N/A')}" for e in environments[:10]]
    ...
    async def merge_merge_request(self, project_id: int | str, mr_iid: int, **kwargs) -> dict[str, Any]:
        data = {k: v for k, v in kwargs.items() if v is not None}
        result = await self.client.merge_merge_request(project_id, mr_iid, data)
        
        summary = f"Successfully merged Merge Request !{mr_iid}"
        key_findings = [f"Status: {result.get('state', 'merged')}", f"Merge Commit SHA: {result.get('merge_commit_sha', 'N/A')[:8]}"]
    ...
    async def bundle_merge_request_context(self, project_id: int | str, mr_iid: int) -> dict[str, Any]:
        """
        ...
        diff_summary = [f"{d['new_path']} ({'added' if d['new_file'] else 'modified'})" for d in diffs]
        ...
        compact_context = {
            ...
            "recent_discussions": [
                {"author": n["author"]["name"], "body": n["body"][:200] + "..." if len(n["body"]) > 200 else n["body"]}
    ...
    async def add_merge_request_discussion_note(self, project_id: int | str, mr_iid: int, discussion_id: str, body: str) -> dict[str, Any]:
        result = await self.client.add_merge_request_discussion_note(project_id, mr_iid, discussion_id, body)
    ...
    async def create_merge_request_discussion(self, project_id: int | str, mr_iid: int, body: str, position: dict[str, Any] | None = None) -> dict[str, Any]:
        result = await self.client.create_merge_request_discussion(project_id, mr_iid, body, position)
        
        summary = f"Successfully created a new discussion on MR !{mr_iid}"
        key_findings = [f"Discussion ID: {result['id']}", f"First Note ID: {result['notes'][0]['id']}"]
    ...
    async def list_merge_request_draft_notes(self, project_id: int | str, mr_iid: int) -> dict[str, Any]:
    ...
    async def delete_merge_request_draft_note(self, project_id: int | str, mr_iid: int, draft_note_id: int) -> dict[str, Any]:
    ...
    async def publish_merge_request_draft_notes(self, project_id: int | str, mr_iid: int) -> dict[str, Any]:
    ...
    async def get_review_summary(self, project_id: int | str, mr_iid: int) -> dict[str, Any]:
    ...
    async def get_unresolved_discussion_digest(self, project_id: int | str, mr_iid: int) -> dict[str, Any]:
        raw = await self.client.get_merge_request_discussions(project_id, mr_iid)
        discussions = [normalize_discussion(d) for d in raw]
    ...
    async def get_suggested_replies(self, project_id: int | str, mr_iid: int) -> dict[str, Any]:
        raw = await self.client.get_merge_request_discussions(project_id, mr_iid)
        discussions = [normalize_discussion(d) for d in raw]
    ...
    async def get_review_digest(self, project_id: int | str, mr_iid: int) -> dict[str, Any]:
        raw = await self.client.get_merge_request_discussions(project_id, mr_iid)
        discussions = [normalize_discussion(d) for d in raw]
    ...
    async def get_draft_reply_plan(self, project_id: int | str, mr_iid: int) -> dict[str, Any]:
        raw = await self.client.get_merge_request_discussions(project_id, mr_iid)
        discussions = [normalize_discussion(d) for d in raw]
    ...
    async def bulk_reply_to_discussions(
        ...
        for r in replies:
            ...
            body = r.get("body", "")
            try:
                await self.client.add_merge_request_discussion_note(project_id, mr_iid, discussion_id, body)
    ...
    async def bulk_resolve_discussions(
        ...
        for did in discussion_ids:
            try:
                await self.client.resolve_merge_request_discussion(project_id, mr_iid, did, resolved)
    ...
    async def update_note(self, project_id: int | str, resource_type: str, resource_iid: int, note_id: int, body: str) -> dict[str, Any]:
        result = await self.client.update_note(project_id, resource_type, resource_iid, note_id, body)
    ...
    async def trigger_pipeline(self, project_id: int | str, ref: str, variables: list[dict[str, str]] | None = None) -> dict[str, Any]:
        ...
        key_findings = [f"Pipeline ID: {result['id']}", f"Status: {result['status']}", f"Web URL: {result['web_url']}"]
    ...
    async def list_merge_request_pipelines(self, project_id: int | str, mr_iid: int) -> dict[str, Any]:
    ...
    async def list_project_members(self, project_id: int | str, query: str | None = None) -> dict[str, Any]:
        ...
        key_findings = [f"{m['name']} (@{m['username']}) - Access Level: {m['access_level']}" for m in members[:10]]
    ...
    async def create_batch_commit(self, project_id: int | str, branch: str, commit_message: str, actions: list[dict[str, Any]], start_branch: str | None = None) -> dict[str, Any]:
        data = {
            "branch": branch,
        ...
        key_findings = [f"Short ID: {result['short_id']}", f"Title: {result['title']}", f"Files impacted: {len(actions)}"]
    ...
    async def get_job_artifacts_archive(self, project_id: int | str, job_id: int) -> dict[str, Any]:
    ...
    async def list_project_variables(self, project_id: int | str) -> dict[str, Any]:
        ...
        key_findings = [f"Key: {v['key']} (Type: {v['variable_type']}, Protected: {v['protected']})" for v in variables[:10]]
    ...
    async def create_project_variable(self, project_id: int | str, key: str, value: str, variable_type: str = "env_var", protected: bool = False, masked: bool = False) -> dict[str, Any]:
        data = {
            "key": key,
            "value": value,
    ...
    async def update_project_variable(self, project_id: int | str, key: str, value: str | None = None, variable_type: str | None = None, protected: bool | None = None, masked: bool | None = None) -> dict[str, Any]:
        data = {}
        if value is not None: data["value"] = value
        if variable_type is not None: data["variable_type"] = variable_type
        if protected is not None: data["protected"] = protected
        if masked is not None: data["masked"] = masked
    ...
    async def bundle_issue_context(self, project_id: int | str, issue_iid: int) -> dict[str, Any]:
        """
    ...
    async def bundle_project_intelligence(self, project_id: int | str) -> dict[str, Any]:
        ...
        dashboard = {
            ...
            "open_merge_requests": [
                {"iid": m["iid"], "title": m["title"], "author": m["author"]["name"], "url": m["web_url"]}
                for m in mrs[:5]
            ],
            "open_issues": [
                {"iid": i["iid"], "title": i["title"], "author": i["author"]["name"], "url": i["web_url"]}
                for i in issues[:5]
            ]
        }
    ...
    async def bundle_pipeline_context(self, project_id: int | str, pipeline_id: int) -> dict[str, Any]:
        """
        ...
        if failed_jobs:
            ...
            try:
                failed_job_analysis = await self.analyze_failed_job(project_id, first_failed_job['id'])
            except Exception:
                pass
        ...
        compact_context = {
            ...
            "job_summary": [
                {"name": j["name"], "status": j["status"], "id": j["id"]}
                for j in jobs
            ],
    ...
    async def triage_issue_locally(self, project_id: int | str, issue_iid: int) -> dict[str, Any]:
        """
        ...
        local_analysis = await self.local_ai.generate_summary(prompt, issue.get("description", "No description provided."))
        ...
        return {
            ...
            "details": {
                ...
                "local_ai_triage": local_analysis
            },
            "next_action": "Review the suggestions and apply appropriate labels or updates."
        }

    async def summarize_mr_discussions_locally(self, project_id: int | str, mr_iid: int) -> dict[str, Any]:
        """
        ...
        return {
            ...
            "key_findings": key_findings,
            "details": {
                "total_comments": len(user_notes),
                "local_ai_summary": local_summary
            },
            "next_action": "Address the critical concerns identified in the summary."
        }

    async def list_vulnerability_findings(self, project_id: int | str, severity: list[str] | None = None, report_type: list[str] | None = None) -> dict[str, Any]:
        """
    ...
    async def check_mr_privacy_locally(self, project_id: int | str, mr_iid: int) -> dict[str, Any]:
        """
