...
class GitLabService:
    ...
    def resolve_url_or_ids(self, url: str | None, project_id: int | str | None, resource_id: int | None) -> tuple[int | str, int | None]:
        """
    ...
    async def list_projects(self, search: str | None = None) -> dict[str, Any]:
        projects = await self.client.list_projects(search=search)
        
        summary = f"Found {len(projects)} projects"
        if search:
            summary += f" matching '{search}'"
        
        key_findings = [f"{p['name']} ({p['path_with_namespace']})" for p in projects[:5]]
        if len(projects) > 5:
    ...
    async def list_project_issues(self, project_id: int, state: str = "opened") -> dict[str, Any]:
    ...
    async def list_project_merge_requests(self, project_id: int, state: str = "opened") -> dict[str, Any]:
    ...
    async def get_issue_details(self, project_id: int, issue_iid: int) -> dict[str, Any]:
    ...
    async def update_issue(self, project_id: int, issue_iid: int, **kwargs) -> dict[str, Any]:
    ...
    async def get_merge_request_details(self, project_id: int, mr_iid: int) -> dict[str, Any]:
    ...
    async def update_merge_request(self, project_id: int, mr_iid: int, **kwargs) -> dict[str, Any]:
        data = {k: v for k, v in kwargs.items() if v is not None}
    ...
    async def approve_merge_request(self, project_id: int, mr_iid: int) -> dict[str, Any]:
    ...
    async def list_issue_notes(self, project_id: int, issue_iid: int) -> dict[str, Any]:
    ...
    async def create_issue_note(self, project_id: int, issue_iid: int, body: str) -> dict[str, Any]:
    ...
    async def list_merge_request_notes(self, project_id: int, mr_iid: int) -> dict[str, Any]:
    ...
    async def get_merge_request_diffs(self, project_id: int, mr_iid: int) -> dict[str, Any]:
        diffs = await self.client.get_merge_request_diffs(project_id, mr_iid)
        
        summary = f"Found {len(diffs)} changed files in Merge Request !{mr_iid}"
    ...
    async def list_repository_files(self, project_id: int, path: str = "", ref: str = "main") -> dict[str, Any]:
    ...
    async def get_file_content(self, project_id: int, file_path: str, ref: str = "main") -> dict[str, Any]:
        import base64
    ...
    async def get_raw_file_content(self, raw_url: str) -> dict[str, Any]:
        content = await self.client.get_raw_file(raw_url)
        filename = raw_url.rstrip("/").split("/")[-1]

        return {
            "summary": f"Retrieved raw content of '{filename}' from {raw_url}",
            "key_findings": [
                f"File: {filename}",
                f"Size: {len(content)} bytes",
                f"Lines: {content.count(chr(10)) + 1}",
            ],
            "details": {"content": content, "url": raw_url},
            "next_action": "Analyze or use the raw file content as needed.",
        }

    async def create_branch(self, project_id: int, branch: str, ref: str) -> dict[str, Any]:
    ...
    async def commit_file(self, project_id: int, file_path: str, branch: str, content: str, commit_message: str, is_new: bool = True) -> dict[str, Any]:
        if is_new:
    ...
    async def search_code(self, project_id: int, search: str) -> dict[str, Any]:
    ...
    async def global_search(self, scope: str, search: str) -> dict[str, Any]:
        results = await self.client.global_search(scope, search)
    ...
    async def list_pipelines(self, project_id: int) -> dict[str, Any]:
    ...
    async def list_pipeline_jobs(self, project_id: int, pipeline_id: int) -> dict[str, Any]:
    ...
    async def get_pipeline_bridges(self, project_id: int, pipeline_id: int) -> dict[str, Any]:
    ...
    async def get_job_log(self, project_id: int, job_id: int) -> dict[str, Any]:
        log_content = await self.client.get_job_log(project_id, job_id)
        log_lines = log_content.splitlines()
        tail_log = "\n".join(log_lines[-100:])
    ...
    async def list_merge_request_discussions(self, project_id: int | str, mr_iid: int) -> dict[str, Any]:
        discussions = await self.client.get_merge_request_discussions(project_id, mr_iid)
        
        summary = f"Found {len(discussions)} discussions for Merge Request !{mr_iid}"
        key_findings = [f"Discussion {d['id'][:8]}: {len(d['notes'])} notes" for d in discussions[:5]]
    ...
    async def bundle_issue_context(self, project_id: int | str, issue_iid: int) -> dict[str, Any]:
        ...
        issue, notes = await asyncio.gather(issue_task, notes_task)
        ...
        return {
            ...
            "key_findings": key_findings,
            "details": compact_context,
            "next_action": "Analyze the bundled context to propose a resolution or ask for more details."
        }
